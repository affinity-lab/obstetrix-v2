...
func (d *DeployModule) Run(ctx context.Context, proj *config.ProjectConfig, sha string, isRollback bool) error {
	...
	// Clean stale framework artifacts and incompatible lockfile before each build.
	// .svelte-kit/ must be regenerated by svelte-kit sync on every deploy.
	// bun.lock v2 (Bun ≥1.2 text format) is unreadable by Bun <1.2; deleting it
	// lets any installed Bun version write a fresh lockfile during install.
	os.RemoveAll(filepath.Join(proj.WorkDir, ".svelte-kit"))
	os.Remove(filepath.Join(proj.WorkDir, "bun.lock"))
	emit("==> build: " + buildCmd)
