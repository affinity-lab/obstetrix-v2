import { c as escape_html, e as ensure_array_like, d as attr, b as stringify } from "../../../chunks/renderer.js";
...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<div class="flex flex-col gap-6"><div class="flex items-center justify-between"><h1 class="text-control-c text-lg font-semibold">backups</h1> `);
    Button($$renderer2, {
      small: true,
      secondary: true,
      onclick: runSystem,
    ...
    if (projects.length === 0) {
      ...
      for (let $$index_1 = 0, $$length = each_array.length; $$index_1 < $$length; $$index_1++) {
        ...
        $$renderer2.push(`<div class="bg-raised border border-base-b rounded-lg overflow-hidden"><div class="flex items-center justify-between px-4 py-3 border-b border-base-b"><a${attr("href", `/backups/${stringify(project.name)}`)} class="text-control-c text-sm font-medium hover:underline">${escape_html(project.name)}</a> `);
        Button($$renderer2, {
          micro: true,
        ...
        if ((backups[project.name] ?? []).length === 0) {
          $$renderer2.push("<!--[0-->");
          $$renderer2.push(`<p class="text-muted-c text-xs px-4 py-3">no backups yet</p>`);
        } else {
          $$renderer2.push("<!--[-1-->");
          $$renderer2.push(`<!--[-->`);
          ...
          for (let $$index = 0, $$length2 = each_array_1.length; $$index < $$length2; $$index++) {
            ...
            $$renderer2.push(`<div class="flex items-center gap-3 px-4 py-2.5 border-b border-base-b last:border-0 text-xs"><span class="text-muted-c flex-1">${escape_html(b.createdAt)}</span> <span class="text-muted-c font-mono">${escape_html(fmt(b.sizeBytes))}</span> <a${attr("href", `/api/backup/download?path=${stringify(encodeURIComponent(b.path))}`)} class="text-muted-c hover:text-control-c" download="">↓ download</a></div>`);
          }
          $$renderer2.push(`<!--]--> `);
          if ((backups[project.name] ?? []).length > 3) {
            $$renderer2.push("<!--[0-->");
            $$renderer2.push(`<a${attr("href", `/backups/${stringify(project.name)}`)} class="block px-4 py-2 text-muted-c text-xs hover:text-control-c hover:bg-secondary/50 transition-colors">view all ${escape_html(backups[project.name].length)} backups →</a>`);
          } else {
            $$renderer2.push("<!--[-1-->");
          }
          $$renderer2.push(`<!--]-->`);
