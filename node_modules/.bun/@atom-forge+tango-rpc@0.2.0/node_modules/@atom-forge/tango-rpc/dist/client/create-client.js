 ...
 */
export function createClient(baseUrl = "/api", options) {
                ...
                if (isResultRequested || isContextRequested) {
                    ...
                    return async (args, options = {}) => {
                        ...
                        if (isDebug) {
                            ...
                            if (ctx.response) {
                                console.log(`%c${duration} %cms / %c${parseFloat(ctx.response.headers.get("X-Tango-Execution-Time") || "0").toFixed(2)} %cms`, "font-weight:800;", "font-weight:200;", "font-weight:800;", "font-weight:200;");
                                console.groupEnd();
                                let color;
...
async function call(baseUrl, ctx) {
    ...
    try {
        ...
        throw new Error(`Failed to decode server response: ${error.message}`);
    }
    if (!response.ok) {
