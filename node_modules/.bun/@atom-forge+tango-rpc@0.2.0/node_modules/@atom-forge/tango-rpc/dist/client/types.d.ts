 ...
 */
type MappedApi<T> = {
    [K in keyof T]: T[K] extends RpcMethodDescriptor ? CallableRpcMethod<GetArgs<T[K]>, GetRet<T[K]>, GetRpcType<T[K]>> : T[K] extends object ? T[K] extends Function | RpcMethodDescriptor ? never : keyof ApiClientDefinition<T[K]> extends never ? never : ApiClientDefinition<T[K]> : never;
};
/**
 ...
 */
type GetRet<T> = T extends {
    implementation: (args: any) => infer R;
} ? UnwrappedPromise<R> : never;
/**
 ...
 */
type CallableRpcMethod<ARGS, RET, Type extends "query" | "command" | "get"> = Type extends "command" ? {
    $command: (args: ARGS, options?: CallOptions) => Promise<RET>;
    $command_ctx: (args: ARGS, options?: CallOptions) => Promise<ClientContext<RET>>;
} : Type extends "query" ? {
    $query: (args: ARGS, options?: CallOptions) => Promise<RET>;
    $query_ctx: (args: ARGS, options?: CallOptions) => Promise<ClientContext<RET>>;
} : Type extends "get" ? {
    $get: (args: ARGS, options?: CallOptions) => Promise<RET>;
    $get_ctx: (args: ARGS, options?: CallOptions) => Promise<ClientContext<RET>>;
} : never;
export {};
