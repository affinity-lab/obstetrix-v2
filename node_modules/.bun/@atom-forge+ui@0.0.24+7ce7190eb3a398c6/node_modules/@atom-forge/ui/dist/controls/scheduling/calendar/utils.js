...
export function dayDiff(startStr, endStr) {
    return Math.round((parseDate(endStr).getTime() - parseDate(startStr).getTime()) / 86400000);
}
export function todayStr() {
    return formatDate(new Date());
}
export function getCalendarWeeks(year, month, firstDayOfWeek) {
    ...
    while (true) {
        ...
        weeks.push(week);
        if (parseDate(week[6]) >= lastOfMonth)
            break;
    }
    return weeks;
}
...
export function isRestDay(dateStr, workdays, extraRest, extraWork) {
    if (extraWork.includes(dateStr))
        return false;
    if (extraRest.map(entryDate).includes(dateStr))
        return true;
    const d = parseDate(dateStr);
    return !workdays.includes(String(d.getDay())); // 0=Sun,1=Mon,...
}
export function getHolidayName(dateStr, extraRest) {
...
export function computeWeekSlots(weekDays, events, wrapMultiDay, resizePreview) {
    ...
    const overlapping = events.filter(e => {
        ...
        if (!wrapMultiDay && effectiveStart < weekStart)
            return false;
...
export function getDayNames(firstDayOfWeek) {
