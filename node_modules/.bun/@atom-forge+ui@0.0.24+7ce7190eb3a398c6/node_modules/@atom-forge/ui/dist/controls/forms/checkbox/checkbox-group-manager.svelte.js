...
export class NamedCheckboxGroupManager {
    masters = new Map();
    values = new Map;
    registerMaster(id, changeState) {
        this.masters.set(id, { changeState });
        this.updateMasterState();
    }
    unregisterMaster(id) { this.masters.delete(id); }
    registerValue(id, value, changeState) {
        this.values.set(id, { value, changeState });
        this.updateMasterState();
    }
    unregisterValue(id) { this.values.delete(id); }
    updateMasterState() {
...
export class CheckboxGroupManager {
    groups = new Map();
    registerMaster(name, id, changeState) {
    ...
    registerValue(name, id, value, changeState) {
        let group = this.groups.get(name);
        if (!group) {
            group = new NamedCheckboxGroupManager();
            this.groups.set(name, group);
...
export const getCheckboxGroupManager = () => getContext(KEY);
export const setCheckboxGroupManager = (manager) => setContext(KEY, manager);
